...
LSF_DOCKER_PRESERVE_ENVIRONMENT=false bsub -q martyomov -G compute-martyomov \
	...
	/bin/bash -c \
	"conda init; source activate /storage1/fs1/martyomov/Active/IndividualBackUps/carisa/conda/envs/reformat-geo ; python3 $WD/scripts/preparation/format_fastq.py $SRR_ACC ${WD}/fastq/${GSM_ACC}/${SRR_ACC}/ $CHEMISTRY" 
