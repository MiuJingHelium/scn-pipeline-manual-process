...
input_obj <- args[1]
name <- args[2] #sample name or GSM accession
outdir <- args[3]

counts <- readRDS(input_obj)
data <- CreateSeuratObject(counts, project=name, min.cells = 3, min.features = 10)
...
ggsave(paste0(outdir,"/plots/",name,"_vln_feature_plots.pdf"), plot=vlnFeaturePlots, width=6, height=4)

umiMtPlot <- FeatureScatter(data, feature1 = "nCount_RNA", feature2 = "percent.mt") + theme(aspect.ratio = 1)
ggsave(paste0(outdir,"/plots/",name,"_umi_mt_plot.pdf"), plot=umiMtPlot, width=6, height=4)

umiFeaturesLog10Plot <- FeatureScatter(data, feature1 = "nCount_RNA_log10", feature2 = "nFeature_RNA_log10") +
  theme(aspect.ratio = 1)
ggsave(paste0(outdir,"/plots/",name,"_umi_features_log10_plot.pdf"), plot=umiFeaturesLog10Plot, width=6, height=4)

umiFeaturesPlot <- FeatureScatter(data, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") + theme(aspect.ratio = 1)
ggsave(paste0(outdir,"/plots/",name,"_umi_features_plot.pdf"), plot=umiFeaturesPlot, width=6, height=4)
...
tryCatch({
  ...
  ggsave(paste0(outdir,"/plots/",name,"_miqc_plot_prob.pdf"), plot=miqcPlotProb, width=6, height=4)
}, error = function(e) {
  message(e)
  pdf(paste0(outdir,"/plots/",name,"_miqc_plot_prob.pdf"), width=6, height=4)
  dev.off()
})

miqcPlotKeep <- FeatureScatter(data, feature1 = "nFeature_RNA", feature2 = "percent.mt", group.by = "miQC.keep") +
  theme(aspect.ratio = 1)
ggsave(paste0(outdir,"/plots/",name,"_miqc_plot_keep.pdf"), plot=miqcPlotKeep, width=6, height=4)
...
ggsave(paste0(outdir,"/plots/",name,"_vln_feature_plots_after_log.pdf"), plot=vlnFeaturePLotsAfterLog, width=6, height=4)

vlnFeaturePlotsAfter <- VlnPlot(data, features = c("nFeature_RNA","nCount_RNA","percent.mt"), pt.size=0.1)
ggsave(paste0(outdir,"/plots/",name,"_vln_feature_plots_after.pdf"), plot=vlnFeaturePlotsAfter, width=6, height=4)

umiMtPlotAfter <- FeatureScatter(data, feature1 = "nCount_RNA", feature2 = "percent.mt") + theme(aspect.ratio = 1)
ggsave(paste0(outdir,"/plots/",name,"_umi_mt_plot_after.pdf"), plot=umiMtPlotAfter, width=6, height=4)

umiFeaturesLog10PlotAfter <- FeatureScatter(data, feature1 = "nCount_RNA_log10", feature2 = "nFeature_RNA_log10") +
  theme(aspect.ratio = 1)
ggsave(paste0(outdir,"/plots/",name,"_umi_features_log10_plot_after.pdf"), plot=umiFeaturesLog10PlotAfter, width=6, height=4)

umiFeaturesPlotAfter <- FeatureScatter(data, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") +
  theme(aspect.ratio = 1)
ggsave(paste0(outdir,"/plots/",name,"_umi_features_plot_after.pdf"), plot=umiFeaturesPlotAfter, width=6, height=4)
...
ggsave(paste0(outdir,"/plots/",name,"_elbow_plot.pdf"), plot=elbowPlot, width=6, height=4)
gc()
...
ggsave(paste0(outdir,"/plots/",name,"_tsne_plot.pdf"), plot=tsnePlot, width=6, height=4)

umapPlot <- DimPlot(data, reduction = "umap") + theme(aspect.ratio = 1)
ggsave(paste0(outdir,"/plots/",name,"_umap_plot.pdf"), plot=umapPlot, width=6, height=4)
gc()

## SAVING: DATASET

write(toJSON(seurat_stats, auto_unbox = T, pretty = T),
      paste0(outdir,"/stats/",name,"_seurat_stats.json"))

saveRDS(data, file = paste0(outdir,"/seurat_objects/",name,"_seurat.rds"))
gc()
