...
awk -F "," 'NR>1{print $11}' $METADATA_TABLE | sort | uniq > $OUTDIR/$GSM_ACC_NAME
# if you want to assign GSM as a specific row of the GSM_ACC_list
# GSM=$(awk 'NR==1{print}' GSM_ACC_list.txt)

while read GSM
do 
	awk -F "," -v GSM=$GSM 'NR>1{ if ($11 == GSM) {print $1} else {next} }' $METADATA_TABLE > $OUTDIR/${GSM}_SRR_ACC_list.txt
